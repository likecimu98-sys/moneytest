...
// ── SUBJECT REGISTRY ── single source of truth for subject name/short/color/inference
const SUBJECT_META = [
  { key: 'История', short: 'История', color: '#446fd4', match: ['истор'] },
  { key: 'Общество', short: 'Общество', color: '#8f762f', match: ['обществ'], aliases: ['Обществознание'] },
  { key: 'Русский', short: 'Русский', color: '#b95757', match: ['рус'] },
  { key: 'Математика', short: 'Матем.', color: '#3f7d5b', match: ['мат'] },
  { key: 'Английский', short: 'Англ.', color: '#6f5ca8', match: ['англ'] },
  { key: 'Физика', short: 'Физика', color: '#2f6f8f', match: ['физ'] },
  { key: 'Химия', short: 'Химия', color: '#9a5fb0', match: ['хим'] },
  { key: 'Биология', short: 'Биол.', color: '#4d8f3f', match: ['биол'] },
  { key: 'География', short: 'Геогр.', color: '#3f8f86', match: ['геогр'] },
  { key: 'Информатика', short: 'Информ.', color: '#52609c', match: ['информ'] },
  { key: 'Литература', short: 'Литер.', color: '#a8643f', match: ['литер'] },
  { key: 'Немецкий', short: 'Нем.', color: '#7d7340', match: ['немец'] },
  { key: 'Французский', short: 'Франц.', color: '#7a5db0', match: ['франц'] },
  { key: 'Испанский', short: 'Исп.', color: '#b0723f', match: ['испан'] },
  { key: 'Китайский', short: 'Кит.', color: '#b04f4f', match: ['кита'] },
  { key: 'Другое', short: 'Другое', color: '#6f7378', match: [] }
];
const SUBJECT_BY_NAME = (() => {
  const map = {};
  SUBJECT_META.forEach(s => {
    map[s.key] = s;
    (s.aliases || []).forEach(alias => { map[alias] = s; });
  });
  return map;
})();
const inferSubject = (text = '') => {
  const t = String(text).toLowerCase();
  const hit = SUBJECT_META.find(s => s.match.some(frag => t.includes(frag)));
  return hit ? hit.key : 'История';
};
const subjectColor = subject => (SUBJECT_BY_NAME[subject] || {}).color || '#777';
const subjectTagText = () => '#fff';
const shortSubject = s => (SUBJECT_BY_NAME[s] || {}).short || s;
    ...
    children: _jsxs("main", {
      ...
      children: [_jsxs("section", {
        ...
        children: [_jsx("div", {
          ...
          children: "Работа над результатом"
        }), _jsxs("div", {
          className: "parent-results-grid",
          children: [_jsxs("div", {
          ...
          className: "parent-results-caption",
          children: (() => {
            const remaining = studyProgress.totalTopics ? Math.max(0, studyProgress.totalTopics - studyProgress.completedTopics) : null;
            if (remaining) return `${studyProgress.subject}: до конца программы осталось ${remaining} ${plural(remaining, 'тема', 'темы', 'тем')}. Идём к цели по плану.`;
            if (attendanceStats.total) return `Системная подготовка идёт стабильно. Спасибо, что доверяете занятия.`;
        ...
        className: "parent-public-footer",
        children: [_jsx("strong", {
...
function App() {
  ...
  const [studentsLimit, setStudentsLimit] = useState(40);
  ...
  const PageToday = () => {
    ...
    const paymentNoticeRows = activeStudents.map(s => {
      const pending = (getParentPortalSettings(s).paymentNotices || []).filter(n => n.status === 'new');
      if (!pending.length) return null;
      return {
        student: s,
        count: pending.length,
        amount: pending.reduce((sum, n) => sum + Number(n.amount || 0), 0)
      };
    }).filter(Boolean);
    const paymentNoticeCount = paymentNoticeRows.reduce((sum, r) => sum + r.count, 0);
    const now = new Date();
    ...
    return _jsxs("div", {
      ...
      children: [currentLesson && _jsx(TimerBanner, {
        ...
        text: "\u0414\u043E\u0431\u0430\u0432\u044C\u0442\u0435 \u0443\u0447\u0435\u043D\u0438\u043A\u0430, \u0437\u0430\u0442\u0435\u043C \u0441\u043E\u0437\u0434\u0430\u0439\u0442\u0435 \u0437\u0430\u043D\u044F\u0442\u0438\u0435 \u0438\u043B\u0438 \u0433\u0440\u0443\u043F\u043F\u0443.",
      ...
      }), paymentNoticeRows.length > 0 && _jsxs("div", {
        className: "card today-notice-card",
        children: [_jsxs("div", {
          className: "today-notice-head",
          children: [_jsx("span", {
            children: "Родители сообщили об оплате"
          }), _jsxs("strong", {
            children: [paymentNoticeCount, " ", plural(paymentNoticeCount, 'заявка', 'заявки', 'заявок')]
          })]
        }), paymentNoticeRows.map(row => _jsxs("div", {
          className: "today-notice-row",
          children: [_jsxs("div", {
            className: "today-notice-info",
            children: [_jsx("strong", {
              children: row.student.name
            }), _jsxs("span", {
              children: ["сообщил(а) об оплате · ", money(row.amount), row.count > 1 ? ` · ${row.count} заявки` : ""]
            })]
          }), _jsx("button", {
            className: "btn btn-sm btn-black",
            onClick: () => setModal({
      ...
      children: [_jsxs("div", {
        ...
        children: [_jsx("div", {
          ...
          children: [_jsx("button", {
            ...
            title: "\u042D\u043A\u0441\u043F\u043E\u0440\u0442 PDF",
            type: "button",
            "aria-label": "\u042D\u043A\u0441\u043F\u043E\u0440\u0442 \u0440\u0430\u0441\u043F\u0438\u0441\u0430\u043D\u0438\u044F \u0432 PDF",
            onClick: () => setModal({
              type: 'schedExport'
            }),
            children: _jsx(IcoPrint, {
  ...
  const PageStudents = () => {
    ...
    return _jsxs("div", {
      ...
      children: [_jsxs("div", {
        ...
        children: [filtered.slice(0, studentsLimit).map(s => {
      ...
      }), filtered.length > studentsLimit && _jsxs("button", {
        type: "button",
        className: "btn btn-white btn-full show-more-btn",
        onClick: () => setStudentsLimit(n => n + 40),
        children: ["Показать ещё (", filtered.length - studentsLimit, ")"]
      })]
      }) : _jsxs(_Fragment, {
  ...
  return _jsxs("div", {
    children: [_jsx(UndoToast, {
      ...
      children: [fabOpen && _jsx("div", {
        ...
        role: "button",
        tabIndex: 0,
        "aria-label": fabOpen ? "Закрыть меню создания" : "Создать",
